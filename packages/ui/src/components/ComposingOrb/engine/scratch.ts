...
//
// Reusable per-runtime scratch storage for the render hot path. Dots live in
// structure-of-arrays Float32Array buffers that are created once per JS
// runtime and reused by every orb: the UI thread runs derived values
// sequentially and each build → record pass completes atomically inside one
// invocation, so a shared buffer can never interleave between orbs.
...
export interface DotBuffer {
  ...
  rs: Float32Array;
  /** Ink value per dot: 0 = darkest ink on paper. */
  ws: Float32Array;
  /** Alpha per dot. */
  as: Float32Array;
...
interface ScratchGlobals {
  __expoThinkingOrbsDyn?: ModeDynamics;
  __expoThinkingOrbsScratch?: DotBuffer;
}

/**
 * Return the runtime's shared {@link ModeDynamics} record, with the given
 * per-frame values written into it. Reused rather than allocated so a frame
 * still allocates only the picture itself.
 */
export function acquireDynamics(
  ...
  "worklet";
  const g = globalThis as unknown as ScratchGlobals;
...
/**
 * Return the runtime's shared {@link DotBuffer}, grown to at least
